...
| FR-003 | Versions are bumped in **lockstep** across the **publishable** set — all four, not the three buildable ones — to the same next prerelease (e.g. `1.1.0-rc.1`), derived from `release-graph.mjs`. | High |
| FR-010 | The bump **rewrites intra-scope peer ranges** to a prerelease-admitting form (`^1.1.0-rc.0`). Measured with semver 7.7.4: `1.1.0-rc.1` does **not** satisfy `^1.0.0`. All four packages peer-depend on each other at `^1.0.0` (styles→tokens, elements→{styles,tokens}, react→elements), so bumping `version` alone would publish an rc set that cannot resolve itself — a successful publish of broken artifacts. | High |
| FR-011 | The bump asserts, as a post-condition, that every intra-scope range is satisfied by the new version — computed via semver, not by inspecting the strings it just wrote. | High |
...
| FR-006 | **REINSTATED AND IMPLEMENTED — operator decision, 2026-09-13, during REL2's pass-3 review.** This row previously read "Withdrawn — moved to REL3", on the reasoning that retiring FR-044 is an ADR amendment rather than a workflow edit. That reasoning was sound and is *why the change needed a decision*; it did not survive contact with a defect this mission itself introduced. REL2 added `publishConfig.registry` (GitHub Packages) to all four manifests, and `publishConfig` **outranks** the npmrc that `setup-node`'s `registry-url` writes — so merging REL2 would have sent prod's tarballs to GitHub Packages while `release.yml` held npmjs credentials. Prod would have failed on its next tag, from a PR that never touched it. Asked to choose between reverting `publishConfig`, warning-only, and making prod consistent, the operator chose to make prod consistent with the standing GitHub-Packages-only ruling. So `release.yml` is retargeted, gains `packages: write` (absent, so it would have 403'd anyway), moves to `GITHUB_TOKEN` (no `NPM_TOKEN` secret has ever existed — verified), and drops `--provenance` + `id-token: write`, which hard-fail on this registry. **FR-044's control is relocated, not retired:** #364 scope item 3 replaces it with `actions/attest-build-provenance`, per the operator amendment of 2026-09-11 on #361 ("GitHub artifact attestations replace npm provenance"). ADR-5 carries a superseding note rather than a rewrite. | High |
