...
| T008 | Compose C5 (GitLab exactly-one group selection) | FR-005, FR-013 | **Blocked on #336, #321** — unstarted |
| T009 | Compose the C6/C7/C8/C9a Installation Detail family (shell + Workspace Scope / Project Routing / Team Accounts sub-navigation) | FR-006, FR-007, FR-008, FR-009, FR-015, FR-016, FR-022, FR-023 | **Blocked on #337** (plus #320/#321 reported-pending for C8's purge button, C9a's disconnect button, and C8's validation inputs — kept blocked per orchestrator ruling 2026-09-10) — unstarted |
| T010 | Compose C9b (Slack channel selection) | FR-010, FR-012 | **Blocked on #321 if the picker includes a filter input** (to be confirmed against the corpus at implementation time) — unstarted |
| T011 | Full-family visual-baseline harvest from CI, final ratchet completion (`expected-parts.json`, `expected-docs.json`, `behaviours.json`, `mutations.json`), and `docs/design-system/using-components.md` ownership section | FR-027, FR-028, FR-029, NFR-008 | **Blocked — transitively, until T008-T010 land** (C-012 keeps delivery to one PR); no baseline is shot or committed this pass regardless, per orchestrator instruction 2026-09-10 — unstarted |

T001 is the source for T002-T003; T002/T003 precede T004/T005; T006/T007 require T004/T005's
rendered output. T008-T010 are independent of each other but each individually blocked as marked.
T011 cannot start until T008-T010 all land, by C-012's one-PR rule.
