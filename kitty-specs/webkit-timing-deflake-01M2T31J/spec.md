...
- **SC-005**: The final `playwright` suite duration is reported alongside the three measured reference runs (25.6 / 26.7 / 22.5 min). A result inside that band passes; one outside it is investigated and explained, not silently accepted. *(Revised mid-mission: the original 5% tolerance was inside the suite's own run-to-run noise — the same defect this repository recorded in #233 and #358 — so it could fire on nothing and prove nothing.)*
