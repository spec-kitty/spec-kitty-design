...
| NFR-004 | No added wall-clock | The mission **reports** the final `playwright` suite duration against the observed band. It does **not** gate on a fixed percentage: pre-mission runs doing comparable work measured **16.5–25.6 min suite self-time (n=11, 55.2% spread)** — so any tolerance tighter than that fires on noise. *(The figures 25.6 / 26.7 / 22.5 and "18.7%" that this row previously carried are WITHDRAWN: no run id existed for any of them and one was a mid-mission lane run. A replacement built on job wall-clock was withdrawn in turn for selection bias. See `scripts/report-playwright-duration.mjs` for the eleven run ids, the comparable-work filter and both corrections.)* A delta beyond the band is a **finding to investigate**, not an automatic failure. | Performance | Medium | Open |
...
- **SC-005**: The final `playwright` suite duration is reported alongside the measured reference band (**16.5-25.6 min suite self-time, n=11**; see `scripts/report-playwright-duration.mjs` for the eleven run ids, the comparable-work filter and the two corrections that produced it -- the original three figures 25.6/26.7/22.5 are WITHDRAWN, they carried no run id and one was a mid-mission lane run). A result inside that band passes; one outside it is investigated and explained, not silently accepted. *(Revised mid-mission: the original 5% tolerance was inside the suite's own run-to-run noise — the same defect this repository recorded in issues 233 and 358 — so it could fire on nothing and prove nothing.)*
