...
2. **A full `playwright` job takes 25.6 min** (the pre-mission run; NFR-004's baseline). Ten sequential CI runs per item is not viable, so NFR-001 means ten **repeats inside one job**. **There is no percentage tolerance.** Three runs of substantially the same suite measured 25.6, 26.7 and 22.5 min — an 18.7% spread — so NFR-004 reports against that **band (22.5–26.7 min)** and flags an outside reading for investigation. Any fixed percentage tighter than the spread would fire on noise.
