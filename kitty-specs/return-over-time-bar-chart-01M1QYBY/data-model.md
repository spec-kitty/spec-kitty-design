...
| `label` | Category/date/category-like text rendered verbatim | non-empty string; literal text only, with no parsing, markup creation, or date interpretation | Consumer |
| `value` | Unformatted magnitude used only for scale derivation | finite number, `>= 0` | Consumer |
| `displayValue` | Already-formatted visible/audible value | non-empty string; literal text only, never parsed, recomputed, or converted to markup | Consumer |
...
generic unavailable state. Validation is all-or-nothing. Valid text remains untrusted: markup-like
characters are emitted through text bindings only and may not create descendant markup, handlers,
resource loads, script execution, or any other side effect.
...
| Focus | outline-based indicator using the approved focus tokens; visible and distinct from selection in normal and forced-colors modes |
| Forced colors | Default and LightMode retain label/value text, bar ownership, baseline, focus outline, and selected-state recoverability without authored color |
