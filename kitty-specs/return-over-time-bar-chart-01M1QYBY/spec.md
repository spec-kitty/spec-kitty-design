...
- **RD-008 — Shared pipeline:** #149 PR #171, merged as `8e654e8`, owns the generic property-only
  manifest/React/Vue seam. #148 is rebased onto train `0fde2ab`, reuses that mechanism without
...
5. **Given** label and display strings containing markup-significant text such as angle brackets,
   ampersands, quotes, and an apparent event-handler payload, **when** rendered, **then** their exact
   literal text is visible in source order, neither field creates descendant markup, and no global
   or DOM side effect occurs.
...
4. **Given** either the Default dark or LightMode story under `forced-colors: active`, **when** a
   reader inspects the chart, **then** label/value text, bar ownership, and the zero baseline remain
   recoverable without depending on authored fill colors.
...
5. **Given** either theme under `forced-colors: active`, **when** a selectable datum receives
   keyboard focus or controlled selection, **then** focus has an outline-based indicator and focus
   and selection remain independently recoverable without color alone.
...
- Long labels wrap inside their own items and preserve full text; they are not ellipsized into a
  changed meaning.
- Markup-significant label/display text remains literal text: it creates no descendant element,
  executable attribute, script, resource request, or other side effect.
...
| FR-004 | Verbatim text | Render label and displayValue as exact literal text, never parse formatted text or create descendant markup from either field, and permit no input-driven side effect. (US1/US2) | High | Open |
...
| FR-015 | Visible focus/selection | Expose focus and selected states with shape/border plus a valid programmatic state, never color alone; under forced colors, keyboard focus must retain an outline-based indicator and remain distinguishable from controlled selection. (US3) | High | Open |
...
| FR-017 | Dark and light | Render an identical semantic signature in default dark and `.sk-light` themes—datum order, exact text, roles, parts, label/value/bar relationships, and ARIA relationships—before permitting only themed color variance. (US1/US2) | High | Open |
...
| NFR-005 | Theme parity | Before color comparison, Default and LightMode have an identical ordered semantic signature covering exact text, roles, parts, datum ownership, label/value/bar relationships, and ARIA relationships. Their semantic data surfaces then resolve to documented themed tokens with at least one key computed color difference. In both themes under forced colors, chart meaning, focus, and selection remain recoverable, with focus represented by an outline. | Visual | High | Open |
| NFR-006 | Cross-browser | Required real-input behavior passes locally available Chromium and Firefox for lane approval and Chromium, Firefox, and WebKit in exact-head PR CI. Local WebKit is additional evidence only through a repository-defined container seam; no such seam exists at planning time and one must not be invented. | Compatibility | High | Open |
...
| NFR-009 | Build budget | A checked-in fail-closed wrapper terminates the Storybook build and exits nonzero at 180 seconds. The Storybook job may retain its deliberate relevant-change predicate, but once that job runs its build step is unconditional and must invoke the wrapper; the wrapper must complete successfully in less than 180 seconds at the final PR head. | Performance | Medium | Open |
...
| C-008 | Serial integration | #149/PR #171 is landed as `8e654e8` and the planning target `mission/return-over-time-bar-chart` records train `0fde2ab`. Before WP01 is first claimed, the orchestrator must fetch, rebase the clean planning target onto latest `origin/train/elements-first`, normalize the human-authored planning range to repository-valid conventional history, and rerun task finalization so `lanes.json.planning_commit_sha` follows the final pre-execution lineage. WP01 executes on `lane-a`; only after its approval may WP03 be claimed on dependent `lane-b`, whose supported allocator must propagate the approved `lane-a` tip. No worker manually rebases, cherry-picks, or consolidates between packages: Spec Kitty 3.2.6rc4 freezes the recorded planning commit after execution starts and owns dependency-tip propagation. After WP03 is approved but before supported lanes→mission→target consolidation captures `baseline_merge_commit`, the orchestrator must fetch and rebase the clean planning target onto latest train and repeat that refresh if train moves. The latest-train target—not either frozen execution lane—determines whether #112's conformance artifact exists. Supported consolidation then creates the PR head; the orchestrator regenerates shared outputs once, commits any changed bytes, and runs the repeatable mandatory gates on that exact head. Never rebase the target after baseline capture; a later train advance is a stale-state recovery condition, not permission to rewrite the recorded baseline. | Delivery | High | Binding |
...
| `LightMode` | `.sk-light` changes only themed colors after an exact semantic-signature match with `Default`; both stories also prove forced-colors focus, selection, and chart recoverability |
...
- **MO-001**: For the approved fixture, all four input label/display pairs render in order and the
  exact ratios are `320/604`, `510/604`, `440/604`, and `1`; a markup-significant fixture preserves
  literal text with zero descendant markup or side effects.
...
- **MO-006**: Dark and LightMode have identical ordered semantic signatures and token-accurate
  series/grid/baseline values, with gold absent from default series fill; forced colors preserves
  outline focus, distinct selection, label/value text, bars, and baseline in both stories.
...
- **MO-009**: All current-train enforced local/CI parity gates—security/lockfile/action pins,
  ...
  exact integrated PR head, with the required WebKit result supplied by PR CI rather than a local
  lane prerequisite.
...
- Elements foundation and existing component/token/gate infrastructure: current mission base is
  `train/elements-first` at inspected commit `0fde2ab`.
...
- Spec Kitty branch roles: `mission/return-over-time-bar-chart` is the planning/landing target;
  ...
  receive approved lane patches until supported final consolidation; WP01 executes on `lane-a`, and
  dependent WP03 executes later on `lane-b` after the supported allocator propagates the approved
  `lane-a` tip. The lanes keep disjoint ownership and remain separate until final consolidation.
