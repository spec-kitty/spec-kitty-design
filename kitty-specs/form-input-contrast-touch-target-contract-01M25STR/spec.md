...
The binding programme decision fixes that the fix is a new token-layer border role, owned by this mission, obligated to clear ≥3:1 against three named surfaces in both themes. It explicitly leaves the exact hex, the exact token name, and whether a width change accompanies the color to this mission. This spec resolves them:
...
2. **Value**: an **alias**, not a new literal — `--sk-border-control: var(--sk-fg-subtle);` in **both** theme blocks (`:root` and `:root[data-theme="light"], .sk-light`), so each theme's `--sk-border-control` resolves to that theme's own `--sk-fg-subtle` (dark `#81818B`, light `#8A8A7E`). `--sk-fg-subtle` is itself already the minimum same-hue lightening the repo derived (#101) to clear 4.5:1 text contrast for tertiary/label text on these same dark surfaces — reusing it here means introducing **zero new hex values** to the palette, matching `adding-a-token.md`'s "aliases are the cheap way to add a category/role" guidance and the existing cross-category alias precedent (`--sk-color-data-grid: var(--sk-border-default)`, `--sk-chart-grid: var(--sk-border-default)`). Measured ratios for the alias (see Non-Functional Requirements, NFR-002, for the full table): the tightest of the six is light-theme vs. `--sk-surface-input` at **3.09:1** — comfortably clear of the 3:1 floor with no rounding ambiguity.
...
**Independent Test**: Read the computed `border-color` of `.sk-input` and of `<sk-form-input>`'s shadow-DOM `.sk-form-input__control` in both themes; compute the contrast ratio against `--sk-surface-page`, `--sk-surface-card`, and `--sk-surface-input`; assert ≥3:1 on all six combinations.
...
- **`sk-textarea` / `.sk-form-textarea__control` now visually diverge from `.sk-input` / `.sk-form-input__control`.** Both textarea rules keep `--sk-border-default` (unchanged, out of scope per the issue's own non-goals). This is a **known, deliberate, and disclosed** consequence of this mission, not a silently introduced inconsistency — recorded here so a reviewer does not mistake it for a missed file. `sk-textarea`'s own contrast/target-size treatment is explicitly flagged as an adjacent instance for a future mission, not pulled into this one.
...
|----|-------|------------|----------|--------|
| FR-001 | New `--sk-border-control` token role, both themes | As a library maintainer, I want a new border role in the Borders block of `packages/tokens/src/tokens.css`, defined in both `:root` and `:root[data-theme="light"], .sk-light`, aliasing `var(--sk-fg-subtle)`, with its derivation and measured ratios recorded in a comment beside the declaration (matching the repo's existing `--sk-fg-subtle`/rose-tint derivation convention), so that the token layer carries one coherent, documented fix. | High | Open |
...
| NFR-002 | Non-text contrast, three surfaces, both themes | The resting-state border color both rules share clears ≥3:1 against `--sk-surface-page`, `--sk-surface-card`, and `--sk-surface-input`, in both themes. Measured for the proposed `--sk-border-control: var(--sk-fg-subtle)` alias: dark `#81818B` — page 5.01:1, card 4.51:1, input 4.28:1; light `#8A8A7E` — page 3.20:1, card 3.49:1, input 3.09:1. All six clear 3:1; the tightest margin is light vs. input at 3.09:1. | Accessibility | High | Open |
...
| C-003 | `sk-textarea`/`sk-form-textarea` untouched | `packages/styles/src/form-field/sk-form-field.css`'s `.sk-textarea` rule and `packages/styles/src/form-textarea/sk-form-textarea.css`'s `.sk-form-textarea__control` rule are not edited by this mission — the resulting visual divergence from `.sk-input`/`.sk-form-input__control` is a disclosed, deliberate consequence (see Edge Cases), not a defect this mission owns. | Technical | High | Open |
...
| C-009 | No new visual-regression baseline wiring | `apps/storybook/src/tests/visual.spec.ts` currently has zero entries for `.sk-input`/`.sk-form-input__control` (verified at this mission's branch point) and this mission does not add any — extending that opt-in, CI-authoritative baseline system to a component it has never covered is a separate, wider decision than this bounded contract. The human visual-review quality gate (Charter Quality Gate 3) plus the new FR-007/FR-008 rendered/forced-colors tests serve as this mission's visual and functional proof instead. | Process | Medium | Open |
...
- **SC-001**: `--sk-border-control` exists in both theme blocks of `packages/tokens/src/tokens.css` and clears ≥3:1 against `--sk-surface-page`, `--sk-surface-card`, and `--sk-surface-input` in both themes (six measured ratios, all ≥3.0, none below 3.09:1).
...
- **SC-005**: The FR-008 forced-colors test passes on both consumption paths, with the control's actual resolved forced-colors border color recorded (not assumed) in the PR.
