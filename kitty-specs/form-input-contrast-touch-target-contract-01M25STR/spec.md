...
| FR-012 | Executable, narrowly-scoped contrast assertion | As a library maintainer, I want a Node-lane test that parses both theme blocks of `packages/tokens/src/tokens.css`, resolves `--sk-border-control` and the three obligated surface tokens per theme, computes the WCAG relative-luminance contrast ratio, and asserts ≥3:1 on all six (theme × surface) combinations, so that the measured table in the PR is verifiable rather than testimonial and a future edit to any of these four tokens cannot silently drop below 3:1 with every other gate green (`bash scripts/check-token-breaking-changes.sh` computes no contrast — see the "Decisions this spec makes" section). This test must be demonstrated red (a deliberate one-step hex nudge to `--sk-border-control`) before it is demonstrated green. Scoped strictly to this mission's own token and three surfaces — it is explicitly **not** the general WCAG 1.4.11 gate #155 raises as an open question, and does not attempt to build one. | High | Open |
...
| NFR-002 | Non-text contrast, three surfaces, both themes | The resting-state border color both rules share clears ≥3:1 against `--sk-surface-page`, `--sk-surface-card`, and `--sk-surface-input`, in both themes. Measured for the chosen `--sk-border-control` literals: dark `#81818B` — page 5.01:1, card 4.51:1, input 4.28:1; light `#7A7A6E` — page 3.98:1, card 4.34:1, input 3.85:1. All six clear 3:1; the tightest margin is light vs. input at 3.85:1. Mechanically enforced by FR-012's executable test, not only recorded. | Accessibility | High | Open |
...
- **SC-001**: `--sk-border-control` exists in both theme blocks of `packages/tokens/src/tokens.css`, as an independently declared literal (not an alias of `--sk-fg-subtle` or any other token), and clears ≥3:1 against `--sk-surface-page`, `--sk-surface-card`, and `--sk-surface-input` in both themes (six measured ratios, all ≥3.0, none below 3.85:1).
