...
| C-002 | Storybook line | `@storybook/web-components-vite` must match the **resolved** Storybook version in the lockfile — currently **10.6.0**. `package.json` now declares `^10.6.0`, matching what its four sibling packages peer-require; earlier drafts of this constraint recorded 10.5.10, which was the resolved version before the framework packages were installed. Pin against the resolved value, not the range. | Technical | High | Open |
...
| C-004 | `LightMode` is out of scope — operator decision | **RESOLVED 2026-09-02 (operator): `LightMode` is scoped OUT of this mission's exit criteria; #93 owns the fix.** #93 is live: `packages/tokens/src/tokens.css:186` defines the light tokens under `:root[data-theme="light"]`, while the stories set `data-theme="light"` on a wrapper `<div>`, which `:root` can never match — so every `LightMode` story renders dark today, before this mission changes anything. This mission therefore must **not** certify "LightMode intact", and ADR-13's confirmation #3 is unsatisfiable until #93 lands. No `LightMode` visual baseline exists, so nothing is frozen into the baseline set either way. Decision ledger: `DM-01M1HXC6WQC90NK940WN2BXR9K`. | Technical | High | Resolved |
