...
corrected below to match the shipped 22-probe table exactly (`scripts/promote-develop.mjs`
itself is authoritative for the literal assertions; this table is authoritative for the *shape*
and *numbering* a reader should expect to find there).
...
mismatched probe named on stderr. `PROBE_FLOOR` in `scripts/promote-develop.mjs` is `22`,
