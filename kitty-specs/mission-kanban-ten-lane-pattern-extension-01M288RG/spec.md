...
| C-007 | Token and logical-property authority | Pattern-local CSS uses existing `--sk-*` tokens and logical properties only — CSS system colours are permitted solely inside the `forced-colors: active` block — and targets only its own `sk-mission-kanban-ten-lane-pattern*` namespace plus parts already recorded in `expected-parts.json`. | Design system | High | Open |
