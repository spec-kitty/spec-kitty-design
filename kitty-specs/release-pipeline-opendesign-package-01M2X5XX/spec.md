...
| FR-003 | The fixture covers **every** component that has a generated static form, with every variant form, derived from the source tree rather than listed by hand. At the base that is 34 components and 159 forms. *(Amended at WP01: 157 are carried; the 2 `boundary-page` forms that compose `<sk-entity-marker>` are not static, so they are left out and named in `DESIGN.md` under FR-004.)* | High |
