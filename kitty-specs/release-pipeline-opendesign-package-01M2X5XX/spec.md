...
| Static forms available | **34** components carry static forms under `packages/styles/src/<name>/*.html` — **159** forms in all. *(Corrected at the REL4 gate: only 11 of the 159 are generated from an element's markup module (ADR-10 §3); the other 148 are the authored forms of styles-only components.)* |
| Not everything has one | of the library's custom elements, **16** have no static form *(corrected from 18: `sk-form-input` and `sk-form-textarea` have theirs under `form-field/`)* (for example `sk-theme-toggle`, `sk-notice`, the charts); they are shadow-DOM-only today |

## Functional requirements

| ID | Requirement | Priority |
|---|---|---|
| FR-001 | A generator in `scripts/` builds the complete OpenDesign package — `manifest.json`, `DESIGN.md`, `tokens.css`, `components.html`, `components.manifest.json`, `USAGE.md` and fonts — into a committed directory in this repository. *(Amended at the REL4 gate: `USAGE.md` is authored, like `DESIGN.md` outside its markers; the build checks both against the generated facts rather than generating them.)* | High |
| FR-002 | `components.html` is assembled from the library's **static forms only**. No custom element, shadow root, `<script>` or runtime dependency may appear in it. *(Amended at the REL4 gate: "generated static forms" would have been 11 forms; the static forms a consumer installs include 148 authored ones, and those are what the package carries.)* | High |
| FR-003 | The fixture covers **every** component that has a static form, with every variant form, derived from the source tree rather than listed by hand. At the base that is 34 components and 159 forms. *(Amended at the REL4 gate — the omission itself was found and implemented in WP01: 157 are carried; the 2 `boundary-page` forms that compose `<sk-entity-marker>` are not static, so they are left out and named in `DESIGN.md` under FR-004. "Generated" dropped with FR-002's amendment.)* | High |
| FR-004 | Components without a static form are **named as excluded** in `DESIGN.md` — derived rather than listed, so OpenDesign's agent knows they exist and why it cannot emit them. *(Amended at plan: not in the components manifest, whose shape is OpenDesign's extractor's own. The reason first recorded here — that OpenDesign regenerates the file on import — was wrong: at discovery it reads the file verbatim (`index.ts:599-609`); corrected at the REL4 gate.)* | High |
| FR-005 | `tokens.css` in the package is **byte-identical** to the token stylesheet the library ships, verified by digest. *(At the REL4 gate: the first build copied the source, `packages/tokens/src/tokens.css`; what `@spec-kitty/tokens` ships is `buildTokensCss(src)`, which adds the no-`data-theme` system fallback. The package now carries the shipped stylesheet, and a probe holds it there.)* | High |
...
| FR-011 | The release path refuses when the committed package is stale, so no release goes out while the package disagrees with the library it publishes. *(Amended at the REL4 gate: first worded "regenerates"; the release does not ship the package, so it checks it, as an exact, unconditional step before any bump.)* | Medium |
...
- **Static-only markup**: `components.html` contains zero custom-element tags, `<script>` elements or declarative shadow roots, asserted by a check with a control that proves the assertion can fail. *(Amended at plan: the original line also forbade `:host`/`::slotted` selectors. 11 of the 34 components ship such rules in the CSS consumers install; they are inert in light DOM. Removing them would teach OpenDesign against a stylesheet nobody ships, so the CSS is inlined verbatim and the constraint applies to markup, where the runtime hazard actually is. Those the package still inlines are named in `DESIGN.md` as a fidelity caveat pending ADR-15 — 10 at merge, because `action-row` is packaged with the static sheet the library already ships for it (REL4 gate pass 2). *(Corrected at the REL4 gate from 13: `boundary-page` and `progress` mention `:host` only in comments.)*)*
...
- **Tokens**: `tokens.css` equals the stylesheet the library publishes (see FR-005).
- **Drift**: `--check` reds on a one-byte change to any generated file and on a new static form that was not regenerated.
- **Consumability**: a real generation on the local OpenDesign instance, using the package, emits library components with **zero invented classes in the library's namespace**, and copies the CSS rules for the classes it uses verbatim — recorded with the prompt, the output, the components used and how both were measured. The API token is read at call time and never written to a file, a commit, a log or this repository. *(Amended at WP03: the first real run met the line as first written — "at least one library component" — while inventing 8 of its 13 `sk-*` classes and hand-writing all component CSS. A bar that passes an unusable output is not the bar. The run also showed the package is only consumable when the agent can read it, so the proof uses a project with the design-system folder linked, and `docs/opendesign-package.md` makes that the documented way to use it.)*
...
- **Static-only means incomplete by design.** 16 elements cannot appear in the fixture until they grow a static form. The exclusion list is the honest record of that, not a defect of this mission. *(Corrected at WP03: the first count, 18, included `sk-form-input` and `sk-form-textarea`, whose static forms live under `form-field/`; the generator derives 16.)*
...
- Adding static forms to the 16 shadow-only elements (separate component work).
