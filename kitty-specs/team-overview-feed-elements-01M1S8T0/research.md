...
mark the mission `BLOCKED`. Recovery then requires an explicit sanctioned operator/maintainer
decision that preserves the held-train and exact-head evidence identities; there is no local
post-consolidation rebase or rebaseline path.
...
7. **Train drift** — rebase the clean target before lane merge and never after exact-head evidence
   ...
   the same post-merge SHA. A later train move makes the mission `BLOCKED`; only an explicit
   sanctioned operator/maintainer recovery may proceed, and the mission must never claim a local
   post-consolidation rebaseline.
