...
This WP has already been claimed, but no product edit was made. Before the worker resumes, the
orchestrator must complete the one-time pre-product recovery in `plan.md` and `tasks.md`: retain the
three pre-repair safety refs; prove the target is normalized, finalized, pushed and local/remote equal
at exact SHA `R`; prove live train is an ancestor of `R`; prove the target and lane worktrees are
clean; prove `kitty/mission-team-overview-feed-elements-01M1S8T0` and
`kitty/mission-team-overview-feed-elements-01M1S8T0-lane-a` still equal
`b05c4907071c578f9004117f0bb6c00bce6d7af3`, lane `HEAD^{tree}` equals `b05c490^{tree}`, and the lane
has no staged, unstaged or product-path delta; then advance the unchecked-out internal branch to `R`
and run the authorized zero-commit `git rebase --onto R b05c490` in lane-a. Both refs and trees must
end exactly at `R`, safety refs and JSONL audit logs must be unchanged, and
`.kittify/workspaces/...` remains CLI-owned. The worker must not perform or repeat this recovery. Once
product editing resumes, the normal no-rebase rule binds.
