...
This planning repair has one bounded exception before product work resumes. WP01 was claimed against
`b05c4907071c578f9004117f0bb6c00bce6d7af3`, but both the internal mission branch and lane-a still
point exactly there, the lane worktree is clean, and neither contains product changes. Preserve the
three pre-repair safety refs, normalize/finalize/push the target, and call its exact repaired SHA `R`.
Before advancing either execution ref, require all of the following: target/local/remote equality at
`R`; live train is an ancestor of `R`; safety refs still resolve to `3b324b2`/`b05c490`; target and
lane worktrees are clean; `kitty/mission-team-overview-feed-elements-01M1S8T0` and
`kitty/mission-team-overview-feed-elements-01M1S8T0-lane-a` still equal `b05c490`; lane `HEAD^{tree}`
equals `b05c490^{tree}`; `git diff --quiet` plus product-path diffs in the lane prove no lane work; the
fresh analysis and finalizer commits are reachable from `R`; and the normalized target range passes
all planning gates. Then, once only, linearly advance the unchecked-out internal branch
`kitty/mission-team-overview-feed-elements-01M1S8T0` to `R` and run a zero-commit
`git rebase --onto R b05c490` in the checked-out lane worktree for
`kitty/mission-team-overview-feed-elements-01M1S8T0-lane-a`. Require both refs to equal `R`, both
trees to equal `R^{tree}`, both worktrees to remain clean, the three safety refs and all JSONL audit
logs to remain unchanged, and the recorded workspace context to remain CLI-owned.
Abort and restore only through the safety refs if any precondition fails. This sanctioned recovery
expires as soon as product editing resumes; it must not be reused as precedent for a later rebase.

1. Complete the bounded pre-product synchronization above; task finalization remains the last
   canonical planning commit and `lanes.json.planning_commit_sha` names its reachable analysis
   parent.
2. Resume WP01, then WP02 and WP03 on the single synchronized lane and run implement → independent
   review serially.
3. After product editing resumes, do not fetch/rebase the active lane; Spec Kitty 3.2.6rc4 freezes
   recorded planning provenance and the ordinary no-rebase-after-execution rule binds again.
