...
### One-time pre-product recovery

WP01 was claimed before the planning repair, but the internal mission branch and lane-a still point
to `b05c4907071c578f9004117f0bb6c00bce6d7af3`, the lane worktree is clean, and no product edit was
made. Before a worker resumes WP01, the mission orchestrator may perform exactly one sanctioned
synchronization. Preserve the three safety refs for the old target, internal branch and lane branch;
normalize/finalize/push the target and name its exact local/remote-equal SHA `R`; prove the held live
train is an ancestor of `R`, both worktrees are clean, the execution refs
`kitty/mission-team-overview-feed-elements-01M1S8T0` and
`kitty/mission-team-overview-feed-elements-01M1S8T0-lane-a` remain exactly `b05c490`, lane
`HEAD^{tree}` equals `b05c490^{tree}`, `git diff --quiet` and product-path comparisons show no lane
work, the fresh analysis/finalizer are reachable, and every planning gate is green. Then linearly
advance the unchecked-out internal mission branch to `R` and run a zero-commit
`git rebase --onto R b05c490` in the checked-out lane worktree. Verify both refs and trees equal `R`,
worktrees remain clean, safety refs and JSONL audit logs are unchanged, and leave
`.kittify/workspaces/...` to supported Spec Kitty CLI ownership—never hand-edit it. Any failed
precondition aborts the recovery. After product editing resumes, this exception expires and the
ordinary no-rebase-after-execution rule is absolute.
...
```text
one-time clean/no-product synchronization to repaired planning target
