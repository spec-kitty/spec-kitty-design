...
|----|-------|-------------|----------|----------|--------|
| NFR-001 | Artifact sizes recorded, not budgeted | Record **raw / minified / minified+gzip** for both artifacts, as the baseline the batch missions are held to. A budget is premature: C-003 forbids migrating a component, so the only artifact is a stub. **The ADRs do not disagree — the basis was missing.** Measured on a representative stub: IIFE **22.8 KB raw / 15.2 KB minified / 5.9 KB min+gzip**; ESM 1.0 KB raw. ADR-10 §2's ~26 KB for one component ≈ **unminified raw**; ADR-8's ~6 KB runtime ≈ **minified+gzip**. An earlier draft recorded the minified figures as "raw" and concluded they contradicted ADR-10; that was inverted and is withdrawn. | Performance | Medium | Open |
