...
| FR-010 | Elements CSS lint rule (ADR-9 confirmation #1) | As a maintainer, I want a lint rule rejecting `:root`, `html`, `body` and `:host-context()` in `packages/elements` CSS, with a red-first test. ADR-9 requires it and no issue #70-#82 claims it; this mission creates the CSS surface it applies to. | Medium | Open |

### Non-Functional Requirements

| ID | Title | Requirement | Category | Priority | Status |
|----|-------|-------------|----------|----------|--------|
| NFR-001 | Artifact sizes recorded, not budgeted | Record both artifacts' sizes **with their basis stated** (raw / minified / gzip), as the baseline the batch missions are held to. A budget is premature here: C-003 forbids migrating a component, so the only artifact is a stub. The ADRs also disagree — ADR-8 says ~6 KB for the Lit runtime, ADR-10 says ~26 KB for one component including it, neither states a basis; a squad measured a representative stub at 15.1 KB raw / 5.8 KB gzip IIFE and 435 B raw ESM, which reconciles with ADR-8 and not ADR-10. | Performance | Medium | Open |
