...
| T005 | Fix the Connectors 200%-zoom test — **initially** the same way as T001 (halved viewport, `deviceScaleFactor: 2`), **then reverted**: CI surfaced a genuine 15px render-height oscillation within a single `toHaveScreenshot` stability loop, specific to that mechanism at that site. Root-caused (web-font swap ruled out by source; no other cause confirmed; local reproduction impossible due to a measured local/CI rendering gap — full record in `research.md`) and resolved per #422's own second option: mechanism reverted to the original `style.zoom`, test renamed to `'Connectors 200% CSS zoom stress — ...'`, matching the honest-naming state of Mission Reading/Work Explorer | FR-005 | Done — resolved by rename, not mechanism change |
