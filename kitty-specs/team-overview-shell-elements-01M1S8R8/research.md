...
### R-07 — presentational behavior maps to SC-010, SC-013 and SC-014
...
Those four elements contribute 12 SC-013/SC-014 arms across eight `(id, subject)` pairs. Add exactly
three SC-010 late-upgrade label arms, registered as `personal-rail.label`,
`context-sidebar.label`, and `button.label`; each reverses only its reflected `label` declaration.
The complete mission addition is therefore exactly 15 registered arms across 11 `(id, subject)`
pairs. Slot assignment, empty slots, native event pass-through, verbatim sync copy, truncation,
responsive bounds and icon accessibility are real regression tests but do not acquire invented ADR
ids. SC-011 includes fallback content; these shells deliberately have none. Button's additional
focus/geometry reversals are demonstrated directly rather than mislabelled in `mutations.json`.
