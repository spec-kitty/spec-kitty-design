...
The no-static-fallback negative invariant is an explicit aggregate anti-vacuity gate, not an
inference from successful generators. WP04 runs its exact 12-path absence command from
`acceptance-matrix.json` immediately after initial generation and repeats it after stable
repeat-generation checks. The enumeration includes all four element `.markup.ts` paths, all four
styles-layer `sk-*.html` paths, and all four styles-layer `index.ts` paths because
`build-element-markup.mjs` and `build-styles-only-markup.mjs` both ignore those orphan HTML files.
