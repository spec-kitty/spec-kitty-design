...
created_at: '2026-09-05T21:23:44.094834+00:00'
subtasks:
- T013
- T014
- T015
...
Immediately after the initial generation/check cycle and again after the stable repeat-generation
cycle, execute this complete negative invariant; a green markup generator is not a substitute:

```bash
test ! -e packages/elements/src/app-shell/sk-app-shell.markup.ts \
  && test ! -e packages/elements/src/personal-rail/sk-personal-rail.markup.ts \
  && test ! -e packages/elements/src/context-sidebar/sk-context-sidebar.markup.ts \
  && test ! -e packages/elements/src/page-header/sk-page-header.markup.ts \
  && test ! -e packages/styles/src/app-shell/sk-app-shell.html \
  && test ! -e packages/styles/src/personal-rail/sk-personal-rail.html \
  && test ! -e packages/styles/src/context-sidebar/sk-context-sidebar.html \
  && test ! -e packages/styles/src/page-header/sk-page-header.html \
  && test ! -e packages/styles/src/app-shell/index.ts \
  && test ! -e packages/styles/src/personal-rail/index.ts \
  && test ! -e packages/styles/src/context-sidebar/index.ts \
  && test ! -e packages/styles/src/page-header/index.ts
```

Both markup generators ignore orphan files at the four styles-layer HTML paths, so only this
explicit enumeration makes their absence non-vacuous.
...
build, demo assembly, and `git diff --check` commands from the planning base. Execute and record the
complete 12-path no-static-fallback command from T015 after initial generation and repeat it after
stable repeat generation; both pass results are required aggregate-gate evidence. Record the local
visual diagnostic separately; it is not a passing gate. Do not claim latest-train,
post-consolidation PR, approved visual, or CI-authoritative exact-head readiness as WP04 evidence. Capture
...
- The aggregate-lane static/type/build/security/release/offline gate list passes at one clean SHA;
  both executions of the 12-path no-static-fallback invariant pass; latest-train consolidation and
  exact-PR gates are explicitly deferred to mission wrap-up.
