...
- **A required behaviour has no applicable component yet.** `sk-stub` has no form
  association and no events. `behaviours.json` is the arbiter: a behaviour with no
  applicable subject is absent from it, not marked skipped and not deferred to a matrix.
...
| FR-010 | The enforced suite contains **zero skipped tests. Full stop.** A behaviour with no applicable subject is simply **not in `behaviours.json`** — it is not expressed as a skip, and it does not depend on FR-012's matrix existing. An author-written reason string is a self-service exemption, the same shape as the marker-anywhere bypass #106 closed in `check-no-css-in-source.mjs`. (An earlier revision routed this through the matrix, which made the whole mission incoherent if FR-012 were dropped; the post-tasks squad caught that the rewrite was claimed but never made.) |
...
| SC-010 | A property assigned **before** the definition loads is applied on upgrade — **fixture-owned**, mutated fixture-locally | remove the fixture's upgrade handling. The `tsconfig.base.json` flag is FR-015's separate node-lane assertion, deliberately **not** a `behaviours.json` id: flipping it shadows every Lit accessor in the fixture, so it is not a guard-5-clean mutation. |
...
| SC-013 | The fixture's `::part()`s are present and targetable from outside — **fixture-owned**, because `packages/elements/src` contains no `part=` and no `@csspart` and the manifest declares **0** `cssParts`, so an elements-owned mutation has no subject and would fail the harness's pattern-not-found guard by construction. The manifest-derived arm is the WP04 ratchet, honestly vacuous at zero and guarded against growing silently. | rename the fixture's internal element carrying the part |
| SC-014 | `shadowRoot.adoptedStyleSheets[0]` **is** `Ctor.styles[0]` — object identity — and the shadow root contains zero `<style>` elements | substitute a Lit `css` CSSResult. NOT "hand-author a `new CSSStyleSheet()`": the generated module already exports exactly that and Lit adopts it by reference, so that substitution preserves identity and leaves the test **green**. Lit adopts `CSSResult.styleSheet`, not the CSSResult, so a `css` template breaks identity. |
| SC-015 | A different-constructor re-`define` warns, leaves the **original** constructor registered, and a same-constructor re-`define` is silent | `if (existing !== ctor) {` → `if (false) {`. NOT "replace `define` with an empty function": that leaves `sk-stub` unregistered, so every other elements-owned test reds too and the harness's collateral bound fails on the mission's own canonical mutation. |
