...
- Seat: a fresh Claude Code `frontend-freddy` seat on Opus, in the dedicated worktree
  `.claude/worktrees/theme-toggle-pass6-remediation` on branch
  `worktree-theme-toggle-pass7-remediation`. It started from clean mission head
  `6c58764c1b582b757b5b4e9d6dcb11d624041281`. Governance was loaded with
  `spec-kitty agent profile show frontend-freddy` and
  `spec-kitty charter context --action implement --json`.
- What the seat did not do:
  - It made no commit, push, stash, reset, rebase or checkout, and changes are left uncommitted.
    The zoom-identity procedure's temporary index wrote only unreferenced blob and tree objects.
  - It invoked none of Spec Kitty `next`, `move-task`, `review`, `accept`, `merge`, `dispatch` or
    `profile-invocation`.
  - It killed no process by pattern.
  - It did not use Storybook port 6006: Playwright ran on `STORYBOOK_PORT=6392` and the capture
    server on 6431.
- F1 (Medium, a manual preference lost across a zero-control gap):
  - The dormant coordinator now remembers the stored value it last read or saved.
  - A control joining an empty document re-reads storage at connect, never its constructor-time
    value. It adopts that read only when it differs from the remembered value. Otherwise, or when
    storage cannot be read, the page's own preference resumes.
  - A refused write is not remembered as saved.
  - The result: the write-denied probe, the stale-detached-instance probe, and a same-tab write
    made while dormant all behave correctly. A same-tab write of exactly the remembered value is
    indistinguishable from no write. That trade-off is documented in `using-components.md`.
- F2 (Low, a synchronous revert left storage disagreeing with the page):
  - A user selection is now persisted once, after its `sk-theme-change` dispatch, as the value the
    page settled on.
  - This does not contradict the rule that an assigned `preference` does not itself persist: the
    handler's assignment writes nothing on its own. The selection's single write now carries the
    settled value.
  - The behaviour is documented.
- F3 (Low, dormant state leaked between tests): each behaviour test now starts from a
  fresh-document-equivalent coordinator.
  - The reset works through the public contract. An assigned System at connect pins the
    preference; a seeded-System connect pins the remembered stored value.
  - Starting-state assertions were added where a premise depends on the start.
  - The earlier first reset drove the preference through storage. One new mutation arm then
    leaked Dark into the event test, which made that arm collateral-dirty. That reset was
    replaced, and the event test is now isolated.
- F4 (Low, seams killed only by accident):
  - (a) The `canReadStorage` seam has one arm per direction.
  - (b) The System-reconnect test installs a fresh media list, which kills a matchMedia-reuse
    mutant.
  - (c) The modern-listener pair check has an arm under the registered SC-012 incomplete-pair
    behaviour.
- F5 and F7: the Playwright spec now checks luminance and non-colour selection.
  - F7 (Medium): all four SC-004 composition cases assert the expected resolved theme, the side of
    the light/dark luminance threshold the surface falls on, and AA contrast.
  - F7 root-only proof: System-light and manual Light prove root-only resolution with no
    `.sk-light` or nested `data-theme` scope.
  - F5 (Low): the greyscale and forced-colours cases now assert that the checked choice differs
    from the unchecked ones in non-colour presentation.
  - The CSS was not changed, because the new assertions passed against the shipped sheet.
  - Discrimination was proven by a probe, recorded in detail in the implementation evidence.
    Under a temporary root-light-but-dark-surface rule plus colour-only checked cues, the old HEAD
    spec's SC-004, greyscale and forced-colours tests passed. The new spec failed those four tests.
- F6 (Low): not folded. Folding the accessor union into `ThemePreference` changes the manifest type
  text, and it puts a bare `ThemePreference` into `vue.d.ts` and `SkThemeToggle.d.ts`, neither of
  which imports it. The fold was reverted and the outputs regenerated byte-identical to the
  pre-fold generation.
- F8 (Low): canonicalized in code rather than by narrowing the docs. The seat chose code because
  the attribute is reflected (`reflect: true`), so a stale invalid literal on the host contradicts
  the reflection contract, and the fix is three lines.
  - After Lit's attribute conversion, an invalid non-null `preference` attribute now requests the
    reflection Lit suppressed.
  - An already-rendered control therefore corrects `sepia` to `system` on its next update, which
    makes the documented claim true.
- F9 (Low): the operational-status exemplar leaves `preference` unbound by default. It renders a
  `preference` attribute only when a caller passes an override. No story passes one any more: each
  state is the stored preference the story's `themePreference` parameter seeds.
- Mutation registry: 263 to 272 arms. One pass-6 arm was re-derived, because its `useOwn` anchor
  no longer exists, and nine are new.
  - Every new and re-derived arm, and every other arm targeting `sk-theme-toggle.ts`,
    `theme-preference.ts` or `operational-status.ts` (27 arms), was applied individually in place
    against both related browser subject files.
  - Each went red on its named test with no marked collateral.
  - Each file was restored byte-identical (sha256).
  - The full 272-arm sweep was not run by this seat; that is the orchestrator's gate.
- Gates:
  - Focused theme browser file: 55/55.
  - Pattern browser file: 14/14.
  - Node theme contract tests: 11/11.
  - `npm test`: 55 files and 730 tests (node 58, chromium 672), zero skipped.
  - Uncached typecheck: 5/5 projects.
  - `quality:all`: 0 errors. It showed 32 warnings, one of them introduced by this pass and then
    fixed; direct eslint of both touched fixture files is clean.
  - Every generator `--check` and repository checker listed in the implementation evidence passed.
  - The `SIZES.md` check passed after a real release-graph build.
  - Playwright, Chromium and Firefox, for the pattern spec plus no-JS: 25 passed, 3 skipped.
  - `git diff --check` is clean.
  - The token catalogue did not change.
- Zoom evidence was recaptured on the final tree with a genuine headed-Chrome procedure, and all
  four images were inspected directly.
  - The capture covers the zoom-200 route and, new this pass, LightMode, each at 100% and 200%.
  - Geometry and the keyboard sequence on the zoom-200 route are numerically identical to pass 6.
  - The PNG bytes differ, because the capture origin port and the pointer position differ.
  - The pass-6 capture helpers were not preserved, so the documented method was re-implemented
    outside the repository.
- Still open for the orchestrator:
  - commit;
  - the full 272-arm mutation sweep;
  - the full Playwright/axe/visual suites;
  - WebKit;
  - commitlint;
  - fresh WP review;
  - the pre-merge squad.
