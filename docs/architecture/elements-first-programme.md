...
**Ratified:** ADR-10's Status is `Accepted (ratified by the operator, 2026-09-02)`, and its Deciders line records that the canonical-markup ruling in §3 was ratified by the operator on 2026-09-02. **ADR-9 and ADR-11 joined it on 2026-09-06** under the #200 ruling, which ratified both by observation rather than by a fresh review of their contents: three `[ENFORCED]` steps across two jobs — two in `lint-code` naming ADR-9, one in `test` naming ADR-11 — plus two charter done-conditions, which name ADR-11's required-behaviours list and not ADR-9, already treated them as binding. Downstream missions build on all three as settled rulings. ADR-12 and ADR-13 remain `Proposed`; #200 ratified two records, not five.
