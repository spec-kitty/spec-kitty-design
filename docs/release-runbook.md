...
|---|---|---|---|
| 1 | Promote the accepted `develop` release candidate to `main` | operator | the release candidate is integrated |
...
`main-is-safe` requires linear history, so step 1 uses squash or rebase rather than a merge
commit. Before merging, verify the PR head has green CI and that both live branch rulesets still
have `bypass_actors: []`; the workflow token cannot verify that write-visible field itself.
