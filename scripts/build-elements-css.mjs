...
const cssText = ${JSON.stringify(stripCssComments(cssText))};
 ...
 */
/*
 * This declaration describes the supported consumer surface: these modules are the
 * browser-facing constructed sheets adopted by the elements and exercised through
 * cssRules/adoptedStyleSheets. The CSSResult branch above is deliberately an import-time SSR
 * fallback only. It lets Lit evaluate an element module without DOM globals; it does not make
 * direct stylesheet inspection or adoption a supported server operation. Widening this type to
 * CSSStyleSheet | CSSResult made every browser consumer re-prove the runtime it already runs in
 * and broke the uncached behaviour-fixture typecheck. Keep the runtime guard truthful in the
 * generated JavaScript and the supported browser contract precise in its declaration.
 */
...
if (selftest) {
  const generated = moduleFor(':host { display: block; }', ['packages/styles/src/probe/sk-probe.css']);
  const executable = generated
    .replace("import { unsafeCSS } from 'lit';\n", '')
    .replace('export default sheet;', 'globalThis.result = sheet;');

  const ssr = {
    unsafeCSS: (value) => ({ kind: 'css-result', cssText: String(value) }),
  };
  runInNewContext(executable, ssr);
  if (ssr.result?.kind !== 'css-result' || ssr.result.cssText !== ':host { display: block; }') {
    console.error('❌ SSR probe did not produce a DOM-free CSSResult fallback.');
    process.exit(1);
  }

  let unsafeCalls = 0;
  class ProbeCSSStyleSheet {
    cssText = '';
    replaceSync(value) {
      this.cssText = value;
    }
  ...
  const browser = {
    CSSStyleSheet: ProbeCSSStyleSheet,
    unsafeCSS: () => {
      unsafeCalls += 1;
      return { kind: 'unexpected-fallback' };
    },
  };
  runInNewContext(executable, browser);
  if (!(browser.result instanceof ProbeCSSStyleSheet) ||
      browser.result.cssText !== ':host { display: block; }' || unsafeCalls !== 0) {
    console.error('❌ Browser probe did not preserve the constructed stylesheet identity path.');
    process.exit(1);
  }

  if (!TYPES.includes('declare const sheet: CSSStyleSheet;') ||
      TYPES.includes('CSSStyleSheet | CSSResult')) {
    console.error('❌ Declaration probe did not preserve the browser-facing stylesheet contract.');
    process.exit(1);
  }

  console.log('✅ build-elements-css self-test passed (3/3).');
  process.exit(0);
}
