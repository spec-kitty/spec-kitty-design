...
//
// Each downstream job must run whenever Storybook succeeds.
...
if (!hasPr) {
  ...
  if ('branches' in pr) {
    ...
    // `develop` is the integration line and `main` is production. Missing either target would
    // allow its PRs to merge without this workflow.
...
// PUSH-SIDE BRANCH COVERAGE. Direct merges into `develop` and production updates on `main`
// must both run the workflow.
const hasPush = onKeys.includes('push');
const pushTrigger = hasPush && on && typeof on === 'object' && !Array.isArray(on) ? (on.push ?? {}) : {};
if (!hasPush) {
  problems.push('the workflow has no `on.push` trigger — merged commits would receive no quality run');
...
if (!gate) problems.push('there is no `gate` job');
  ...
  const REQUIRED_LINT = [
    // REL2 (#363), registered WITH the gate rather than after it — #74 shipped a gate with no
