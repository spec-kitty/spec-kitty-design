...
function render(name) {
  ...
  // ADR-10's "Styles-only components are a class, not a fixed exception count" section is the
  // citation for every styles-only component EXCEPT form-field, segmented-choice, and public-header. #141 recorded
  // form-field's distinct element-path decision. #270 owns segmented-choice's distinct decision to
  // keep naming, pressed state, exclusivity, and activation on a native consumer-authored button
  // group. Do not attribute either component-specific rationale to ADR-10's general class ruling.
  const adrNote =
    name === 'form-field'
...
//
// ${name} is deliberately styles-only — it has no custom element. ${adrNote}
${body}
`;
}
