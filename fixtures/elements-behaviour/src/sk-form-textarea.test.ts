...
import '../../../packages/elements/src/form-textarea/sk-form-textarea.js';

/**
 * <sk-form-textarea> — the same four behaviours against the multi-line control.
 *
 * DELIBERATELY A SEPARATE FILE WITH ITS OWN ANCHORS — but not for the reason this comment
 * originally gave, which a pre-merge lens corrected.
 *
 * It said guard 5 forces separate test FILES. It does not: `suite-selftest.mjs` narrows by file
 * (`t.file.endsWith(m.subject)`), so both elements could live in one file and a mutation in
 * `sk-form-input.ts` would still red only its own variant. What forces per-element files is
 * `tests/node/config-contract.test.ts`, which requires a behaviour subject's file to be ABOUT
 * the element it names.
 *
 * What guard 5 genuinely forces is per-element ANCHORS — a shared line in the element SOURCE
 * reds both elements' `[SC-00x]` tests at once and is rejected as collateral. That was measured
 * at 8 of 37 mutations failing against 37/37 clean, and it is a different claim.
 *
