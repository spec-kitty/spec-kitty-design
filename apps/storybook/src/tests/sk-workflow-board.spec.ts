...
const declaredViewport: Record<(typeof boardStories)[number], { width: number; height: number }> = {
...
async function openBoard(page: Page, id: (typeof boardStories)[number]): Promise<LoadedStory> {
  await page.setViewportSize(declaredViewport[id]);
  return openStory(page, BOARD_STORY_PREFIX, id, '.sk-workflow-board');
}

function readGeneratedFixtures(path: string): Array<{ name: string; html: string }> {
  ...
  if (fixtures.length === 0) throw new Error(`no generated fixtures parsed from ${path}`);
  return fixtures;
}
...
async function pageGeometry(page: Page) {
  return page.evaluate(() => {
    ...
    return { clientWidth: documentScroller.clientWidth, scrollWidth: documentScroller.scrollWidth };
  });
}
...
async function assertNativeLaneContract(root: Locator): Promise<void> {
  ...
  for (let index = 0; index < laneCount; index += 1) {
    ...
    expect(await title.evaluate((node) => /^H[2-6]$/.test(node.tagName))).toBe(true);
    await expect(pageLocatorByRole(lane, 'region', await title.innerText())).toHaveCount(1);
    ...
    await expect(list.getByRole('listitem')).toHaveCount(await directItems.count());
    const count = Number((await lane.locator('.sk-workflow-lane__count').innerText()).trim());
    expect(count).toBe(await directItems.count());
  }
}

function pageLocatorByRole(root: Locator, role: 'region', name: string): Locator {
...
test.describe('workflow board source and distribution contract', () => {
  test('the public CSS inventory is exactly the seven approved classes and contains no adjacent behavior', () => {
    ...
    const classes = [...new Set([...code.matchAll(/\.((?:sk-workflow-(?:board|lane))[a-z0-9_-]*)/g)].map((match) => match[1]))].sort();
    expect(classes).toEqual([...ALLOWED_CLASSES].sort());
    expect(code).not.toMatch(/\b(?:content|order|transition|animation|scroll-behavior)\s*:/);
    expect(code).not.toMatch(/forced-color-adjust\s*:\s*none/);
    expect(code).not.toMatch(/(?:\.sk-light|data-theme|:root|:host-context)/);
    expect(code).not.toMatch(/(?:planned|doing|review|approved|done|kanban|work-package-card|workflow-item)/i);
  });
  ...
  test('the surface remains styles-only and does not enter element, wrapper, manifest, or behavior ownership', () => {
    ...
    for (const path of ['packages/elements/custom-elements.json', 'expected-parts.json', 'expected-docs.json', 'behaviours.json', 'mutations.json']) {
      expect(readFileSync(path, 'utf8')).not.toMatch(/sk-workflow-(?:board|lane)/);
    }
  });

  test('one measured, theme-invariant layout token backs every lane minimum', () => {
    const tokens = readFileSync(TOKEN_SOURCE, 'utf8');
    const declarations = [...tokens.matchAll(/--sk-layout-workflow-lane-min-inline-size:\s*([^;]+);/g)].map((match) => match[1].trim());
    expect(declarations).toHaveLength(2);
    expect(new Set(declarations).size).toBe(1);

    const laneCss = stripComments(readFileSync(LANE_CSS, 'utf8'));
    expect(laneCss).toMatch(/min-inline-size:\s*var\(--sk-layout-workflow-lane-min-inline-size\)/);
    expect(laneCss).not.toMatch(/var\(--sk-layout-(?:personal-rail|context-sidebar)/);
  });
  ...
  test('generated fixtures, root exports, and package subpaths expose both styles-only families', () => {
    ...
    expect(root).toContain("export * from './workflow-board/index';");
    expect(root).toContain("export * from './workflow-lane/index';");
    const packageJson = JSON.parse(readFileSync(STYLES_PACKAGE, 'utf8')) as { exports: Record<string, unknown> };
    expect(packageJson.exports['./workflow-board/*']).toBe('./dist/workflow-board/*');
    expect(packageJson.exports['./workflow-lane/*']).toBe('./dist/workflow-lane/*');
  });

  test('authored fixtures forge no list/grid semantics and generate no accessible text', () => {
    const markup = [...readGeneratedFixtures(BOARD_BARREL), ...readGeneratedFixtures(LANE_BARREL)]
      .map(({ html }) => html)
      .join('\n');
    expect(markup).not.toMatch(/role="(?:grid|row|list|listitem|listbox|option)"/);
...
test.describe('workflow board stories are non-vacuous and console-clean', () => {
  for (const id of boardStories) {
    test(`${id} renders a visible board without browser errors or page overflow`, async ({ page }) => {
  ...
  for (const id of ['default', 'empty', 'light-mode'] as const) {
    test(`lane ${id} renders a visible native lane without browser errors`, async ({ page }) => {
      const loaded = await openStory(page, LANE_STORY_PREFIX, id, '.sk-workflow-lane');
      expect(loaded.consoleErrors).toEqual([]);
      expect(loaded.pageErrors).toEqual([]);
...
test.describe('native sections, lists, counts, and source order', () => {
  test('Populated exposes five named native lanes and ordered lists in consumer order', async ({ page }) => {
    const { root } = await openBoard(page, 'populated');
    await assertNativeLaneContract(root);
    await expect(root.locator('.sk-workflow-lane__title')).toHaveText(['Planned', 'Doing', 'In review', 'Approved', 'Done']);
    await expect(root.getByRole('list')).toHaveCount(5);
  });

  test('AllEmpty keeps five zero-item lists and five visible sibling empty treatments', async ({ page }) => {
    ...
    for (let index = 0; index < 5; index += 1) {
      expect(await emptyStates.nth(index).evaluate((node) => node.previousElementSibling?.tagName)).toBe('OL');
      await expect(emptyStates.nth(index)).toBeVisible();
    }
  });

  test('OneEmptyLane has exactly one empty list and every supplied count matches cardinality', async ({ page }) => {
    const { root } = await openBoard(page, 'one-empty-lane');
    await assertNativeLaneContract(root);
    const lengths = await root.locator('.sk-workflow-lane__list').evaluateAll((lists) =>
  ...
  test('FiftyItems exposes fifty direct native items in unmodified source order and supplied count 50', async ({ page }) => {
    ...
    expect(await items.allTextContents()).toEqual(Array.from({ length: 50 }, (_, index) => `Work package ${String(index + 1).padStart(2, '0')}`));
  });
});

test.describe('conditional overflow semantics and keyboard operation', () => {
  test('Populated is one genuinely overflowing named region with a complete triad', async ({ page }) => {
    const { root } = await openBoard(page, 'populated');
    const scroller = root.locator('.sk-workflow-board__scroller');
    const geometry = await scroller.evaluate((node) => ({ clientWidth: node.clientWidth, scrollWidth: node.scrollWidth }));
    expect(geometry.scrollWidth).toBeGreaterThan(geometry.clientWidth);
    await expect(scroller).toHaveAttribute('role', 'region');
    await expect(scroller).toHaveAttribute('aria-labelledby', 'workflow-board-populated-title');
    await expect(scroller).not.toHaveAttribute('aria-label', /.+/);
    await expect(scroller).toHaveAttribute('tabindex', '0');
    await expect(page.getByRole('region', { name: 'Work Packages', exact: true })).toHaveCount(1);
  });

  for (const id of ['fitting', 'single-lane-narrow'] as const) {
    test(`${id} fits and omits role, name, and tabindex together`, async ({ page }) => {
  ...
  test('focused overflow shows an outline and ArrowRight/ArrowLeft scroll without losing focus', async ({ page }) => {
    const { root } = await openBoard(page, 'populated');
    const scroller = root.locator('.sk-workflow-board__scroller');
    ...
    await expect.poll(() => scroller.evaluate((node) => node.scrollLeft)).toBeLessThan(movedRight);
    await expect(scroller).toBeFocused();
  });
});

test.describe('calibrated geometry, themes, and forced colors', () => {
  test('the 320px single-lane composition fits and resolves the measured lane token', async ({ page }) => {
    ...
    const values = await root.locator('.sk-workflow-lane').evaluate((lane) => ({
      ...
      token: getComputedStyle(lane).getPropertyValue('--sk-layout-workflow-lane-min-inline-size').trim(),
    }));
    expect(values.laneMinimum).toBe(values.token);
    expect(values.laneMinimum).not.toBe('');
  });

  test('long lane and item labels wrap without overlap or lane-internal overflow', async ({ page }) => {
    const { root } = await openBoard(page, 'long-labels-and-items');
    for (const lane of await root.locator('.sk-workflow-lane').all()) {
      const laneGeometry = await lane.evaluate((node) => ({ clientWidth: node.clientWidth, scrollWidth: node.scrollWidth }));
      expect(laneGeometry.scrollWidth).toBeLessThanOrEqual(laneGeometry.clientWidth);
      ...
      const boxes = await Promise.all([header.boundingBox(), title.boundingBox(), count.boundingBox()]);
    ...
    expect(targetBox!.x).toBeGreaterThanOrEqual(scrollerBox!.x);
    expect(targetBox!.x + targetBox!.width).toBeLessThanOrEqual(scrollerBox!.x + scrollerBox!.width);
  });

  test('LightMode changes real token-dependent values while preserving semantic markup', async ({ page }) => {
    const dark = await openBoard(page, 'default-dark');
    const darkHtml = await dark.root.innerHTML();
    const darkStyle = await dark.root.locator('.sk-workflow-lane').first().evaluate((node) => {
      const style = getComputedStyle(node);
    ...
    await expect(light.root.locator('xpath=ancestor::*[contains(concat(" ", normalize-space(@class), " "), " sk-light ")]')).toHaveCount(1);
    expect(await light.root.innerHTML()).toBe(darkHtml);
    const lightStyle = await light.root.locator('.sk-workflow-lane').first().evaluate((node) => {
      const style = getComputedStyle(node);
      return { background: style.backgroundColor, border: style.borderColor, color: style.color };
    });
  ...
  test('forced colors preserves lane boundaries, empty treatment, focus, names, counts, and containment', async ({ page, browserName }) => {
    ...
    const laneStyle = await root.locator('.sk-workflow-lane').first().evaluate((node) => {
      const style = getComputedStyle(node);
    ...
    await page.locator('body').press('Tab');
    await expect(scroller).toBeFocused();
    expect(await scroller.evaluate((node) => Number.parseFloat(getComputedStyle(node).outlineWidth))).toBeGreaterThan(0);
    const geometry = await pageGeometry(page);
    expect(geometry.scrollWidth).toBeLessThanOrEqual(geometry.clientWidth);
  });
});
