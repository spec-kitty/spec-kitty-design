...
import sheet from './sk-form-textarea.css.js';
 ...
 */
export class SkFormTextarea extends FormControlBase {
  static styles = [sheet];
  ...
  static properties = {
    ...
    // exclusion is ours to get right or wrong. `<sk-form-textarea disabled>` in markup still works
  ...
  render() {
    const describedBy = [this.description ? 'description' : '', this.invalid ? this.errorId : '']
      .filter(Boolean)
      .join(' ');
    return html`<div part="field" class="sk-form-textarea">
      <label part="label" class="sk-form-textarea__label" for="control">${this.label}</label>
      <textarea
        part="control"
        class="sk-form-textarea__control"
        id="control"
        rows=${this.rows}
        .value=${this.value}
      ...
      ></textarea>
      ${this.description
        ? html`<span part="description" class="sk-form-textarea__description" id="description"
...
define('sk-form-textarea', SkFormTextarea);
