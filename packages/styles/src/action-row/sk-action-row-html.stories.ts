    ...
    docs: {
      description: {
        component:
          ...
          'ADR-15\'s two-element wrapper, never a single-element collapse. The wrapper\'s CSS is ' +
          'NOT shipped by any package stylesheet in this mission (that is #309\'s scope); a real ' +
          'consumer authors `.sk-action-row-host { display: block; min-width: 0; ' +
          'container-type: inline-size; }` themselves, word for word from `sk-action-row.css`\'s ' +
          'header comment, until #309 lands. This Storybook canvas does the same in the docs ' +
          'below — no host rule reaches @spec-kitty/styles from here.',
      },
    },
  },
};
