...
### Invocation commit-link correction

The closed continuation Op `01M218FT9EJX2RT0T8W3FWN0E4` was accidentally given the nonexistent
commit string `ef7a907b067bc60527f6c009d9e6f96a69180d2b` rather than reading the completed commit
from Git first. The append-only invocation log is preserved. The actual branch-strategy commit is
`ef7a9078928fd6922b08a315141765a914e9d4b2`, verified with `git rev-parse HEAD`; correction Op
`01M218KGBYHNVNJ84KD21ZZ1N5` records the authoritative follow-up head. The incorrect link must not
be used as evidence.
