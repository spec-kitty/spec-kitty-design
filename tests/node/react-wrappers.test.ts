...
// coverage. At the time, it cost 5.7s of the node lane against `suite-budget.json`'s then-25s
// `ceilingSeconds` for `npm run test`, which was 23% of a ceiling that mission would then have
// had to re-measure to justify. `--check` stays: it is the cheap arm and the one a developer
// wants locally.
