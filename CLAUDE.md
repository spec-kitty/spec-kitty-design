...
5. **Conventional commits** with scopes: `tokens`, `storybook`, `doctrine`, `ci`, `docs`, `release`, `deps`, `security`, `styles`, `elements`, `react`, `acceptance`, `merge`, `team-overview`. Note the Spec Kitty CLI's own bookkeeping commits are exempted by narrowly anchored patterns in `commitlint.config.cjs`; yours are not. Subject lowercase. `commitlint` runs on PRs.
